...
namespace BiblioTec.Controllers
{
    ...
    public class ConfiguracionController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Create(ConfiguracionDto dto)
        {
            try
            {
                var config = await _configuracionService.UpdateAsync(dto);
                return CreatedAtAction(nameof(GetAll),
                    new { success = true, data = config });
            }
            catch (InvalidOperationException ex)
            {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, ConfiguracionDto dto)
        {
            try
            {
                var config = await _configuracionService.UpdateAsync(dto);
        ...
        {
            return BadRequest(new { success = false, message = "No se puede eliminar la configuración del sistema." });
        }
    }
}
